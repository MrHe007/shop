...
public class GoodsHandle {
    ...
    public Goods findById(int id) throws Exception {
        ...
        if (rs.next()) {
            ...
            goods.setTag(rs.getInt(11));                // 设置good 的出售方式
        }
    ...
    public boolean doCreate(Goods good) throws Exception {
        boolean flag = false;
        String sql = "INSERT INTO `goods`(name,price,image,content,status,id,type_id,producter_id,create_date,num,tag) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
        this.pstmt = this.conn.prepareStatement(sql);
    ...
    public List<Goods> findAll(IntHolder num,int limitMin,int perPage) throws Exception {
        ...
        String sql = "SELECT SQL_CALC_FOUND_ROWS id,num,content,type_id,image,producter_id,price,create_date,name,tag from goods  where status=? order by create_date desc limit ?,?";
        this.pstmt = this.conn.prepareStatement(sql);
        ...
        while (rs.next()) {
            ...
            good.setTag(rs.getInt(10));             // 设置good 的出售方式
    ...
    public List<Goods> findAllNotAuditing() throws Exception {
        List<Goods> all = new ArrayList<Goods>();
        String sql = "SELECT id,num,content,type_id,image,producter_id,price,name,create_date,tag from goods where status=1";
        this.pstmt = this.conn.prepareStatement(sql);
        ResultSet rs = this.pstmt.executeQuery();
        Goods good = null;
        while (rs.next()) {
    ...
    public boolean doUpdate(Goods good) throws Exception {
        boolean flag = false;
        String sql = "update goods set id=?,image=?,type_id=?,name=?,num=?,price=?,status=?,content=?,producter_id=?,create_date=?,tag=? where id=?";
        this.pstmt = this.conn.prepareStatement(sql);
    ...
    public List<Goods> findByCeta(int cetaId,IntHolder num,int limitMin,int perPage) throws Exception {
        List<Goods> all = new ArrayList<Goods>();
        String sql = "SELECT SQL_CALC_FOUND_ROWS id,num,content,type_id,image,producter_id,price,name,create_date,tag from goods where status=2 and type_id="+cetaId+" order by create_date desc limit ?,?";
        this.pstmt = this.conn.prepareStatement(sql);
    ...
    public List<Goods> findByKey(String key) throws Exception {
        List<Goods> all = new ArrayList<Goods>();
        String sql = "SELECT id,num,content,type_id,image,producter_id,price,name,create_date,tag from goods where status=2 and name like ? order by create_date desc";
        this.pstmt = this.conn.prepareStatement(sql);
    ...
    public List<Goods> findByUserId(int userId) throws Exception {
        List<Goods> all = new ArrayList<Goods>();
        String sql = "SELECT id,num,content,type_id,image,producter_id,price,name,create_date,status,tag from goods where (status=2 or status=4) and producter_id="+userId+" order by create_date desc";
        this.pstmt = this.conn.prepareStatement(sql);
        ResultSet rs = this.pstmt.executeQuery();
        Goods good = null;
        while (rs.next()){
